...
class DagFileProcessor(LoggingMixin):
    ...
    def manage_slas(self, dag: DAG, session: Session = None) -> None:
        ...
        qry = (
            session.query(TI.task_id, func.max(DR.execution_date).label('max_ti'), TI.start_date)
            .join(TI.dag_run)
        ...
        max_tis: Iterator[TI] = (
            ...
            .filter(
                ...
                TI.start_date == qry.c.start_date,
            )
        )
        ...
        if "sla" in dag.default_args:
            ...
            for ti in max_tis:
                ...
                task_sla_check = ti.start_date + task.sla < ts
                dag_sla_check = ti.execution_date + dag.default_args["sla"] < ts
